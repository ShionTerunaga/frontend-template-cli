...
//#endregion
//#region node_modules/.pnpm/ts-shared@https+++codeload.github.com+ShionTerunaga+ts-shared+tar.gz+d482a716f2768dcdc1_14eaddaf2f8dec0b7a2f389c6c0cc029/node_modules/ts-shared/dist/index.mjs
...
var require_prompt$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	var Prompt = class extends EventEmitter$2 {
		constructor(opts = {}) {
			super();
...
var { program, createCommand, createArgument, createOption, CommanderError, InvalidArgumentError, InvalidOptionArgumentError, Command, Argument, Option, Help } = (/* @__PURE__ */ __toESM(require_commander(), 1)).default;
//#endregion
...
async function getCurrentVersion() {
	...
	if (optionVersion.isNone) return createNg(/* @__PURE__ */ new Error("version is not found in version.json"));
...
async function nameCommand(optionName) {
	...
	const response = await checkPromiseReturn({
		...
		err: (e) => {
			if (e instanceof Error) return createNg(/* @__PURE__ */ new Error(`Prompt failed: ${e.message}`));
			return createNg(/* @__PURE__ */ new Error("Prompt failed: Unknown error"));
		}
	});
...
async function copy(src, dest, { cwd, rename = identity, parents = true }) {
	...
	const sourceFiles = await checkPromiseReturn({
		...
		err: () => createNg(/* @__PURE__ */ new Error("Failed to glob source files"))
	});
...
async function typescriptTemplateInstall({ root, appName, material }) {
	...
	const exists = await checkPromiseVoid({
		...
		err: (e) => {
			if (e instanceof Error) return createNg(/* @__PURE__ */ new Error(`Failed to access package.json: ${e.message}`));
			return createNg(/* @__PURE__ */ new Error("Failed to access package.json: Unknown error"));
		}
	});
	...
	const writeResult = await checkPromiseVoid({
		...
		err: () => createNg(/* @__PURE__ */ new Error(`Failed to update package.json name`))
	});
...
var optionReactFramework = (async function() {
	return await optionCommand((await commanderCore).opts().reactFramework);
})();
var optionVueFramework = (async function() {
	return await optionCommand((await commanderCore).opts().vueFramework);
})();
var optionTechStack = (async function() {
	return await optionCommand((await commanderCore).opts().techStack);
})();
...
var REPOSITORY_NAME = "frontend-template-cli";
...
async function getLatestVersion() {
	...
	return checkPromiseReturn({
		...
		err: (error) => error instanceof Error ? createNg(/* @__PURE__ */ new Error(`Failed to resolve the latest repository tag: ${error.message}`)) : createNg(/* @__PURE__ */ new Error("Failed to resolve the latest repository tag"))
	});
}
//#endregion
...
async function checkCliVersion() {
	...
	const currentVersionResult = await getCurrentVersion();
	if (currentVersionResult.isErr) return currentVersionResult;
	const currentVersion = currentVersionResult.value;
	...
	console.log((0, import_picocolors_browser.red)(`A newer version of create-frontend-template is available. current: v${currentVersion}, latest: v${latestVersion}`));
	...
	console.log((0, import_picocolors_browser.bold)(`${(0, import_picocolors_browser.green)(INSTALL_COMMAND)}`));
	console.log();
	process.exit(0);
}
