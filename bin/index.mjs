...
//#endregion
//#region src/shared/template-cli.ts
var templateCliConfig = {
	react: {
		title: "React",
		command: "npx",
...
var techStacks = Object.keys(templateCliConfig);
//#endregion
//#region src/command/tech-stack.ts
function isTechStack(value) {
	return typeof value === "string" && techStacks.includes(value);
}
async function techStackCommand(currentVersion) {
	const optionTech = new Command("create-frontend-template").version(currentVersion, "-v, --version", "output the current version").helpOption("-h, --help", "display help for command").option("-t, --tech-stack <techStack>", "specify the tech stack (react | vue)").parse(process.argv).opts().techStack;
	if (optionTech !== void 0) {
		if (isTechStack(optionTech)) return optionTech;
...
async function fetchLatestTagVersion() {
	...
	if (!Array.isArray(payload) || payload.length === 0) throw new Error("GitHub tags response is empty");
	const [latestTag] = payload;
	if (typeof latestTag !== "object" || latestTag === null || !("name" in latestTag) || typeof latestTag.name !== "string") throw new Error("GitHub tags response does not contain a valid tag name");
...
async function getLatestVersion() {
	try {
		...
		throw new Error(`Failed to resolve the latest repository tag${message}`);
	}
}
//#endregion
//#region src/run-template-cli.ts
async function runTemplateCli(techStack) {
	...
	await new Promise((resolve, reject) => {
		...
		child.once("close", (code, signal) => {
			...
			reject(/* @__PURE__ */ new Error(`${command} terminated with ${reason}`));
		});
	});
}
//#endregion
...
async function run() {
	...
	await runTemplateCli(await techStackCommand(currentVersion));
}
async function getCurrentVersion() {
	...
	if (typeof versionJson.version !== "string") throw new Error("version is not found in version.json");
...
async function checkCliVersion(currentVersion) {
	...
	if (pD(shouldContinue)) process.exit(1);
...
run().catch((error) => {
	...
	console.error((0, import_picocolors_browser.red)(message));
	process.exitCode = 1;
});
//#endregion
